...
class BosClient extends BceBaseClient {
	...
	public function fetchObject(
		...
		$this->populateRequestHeadersWithOptions($headers, $options);
		list($config, $storageClass, $fetchMode) = $this->parseOptions(
			$options,
			BosOptions::CONFIG,
			BosOptions::STORAGE_CLASS,
	...
	public function uploadPartCopy(
		...
		if (
			$partNumber < BosClient::MIN_PART_NUMBER
	...
	public function uploadPart(
		...
		list($config, $contentCRC32, $contentSHA256) = $this->parseOptions(
			$options,
			BosOptions::CONFIG,
	...
	public function putSuperObjectFromFile(
		...
		while ($bytesLeft > 0) {
			...
			try {
				$response = $this->uploadPartFromFile(
					$bucketName,
					$key,
					$uploadId,
					$partNumber,
